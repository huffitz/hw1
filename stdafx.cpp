...
bool create_bob_txt()
{
	...
	if (!SUCCEEDED(StringCbPrintfW(
		string_buf,
		sizeof(string_buf),
		L"Best of the Best 베스트 오브 더 베스트")))
	{
	...
	{
		print("err, CreateFileW(%ws) failed, gle = %u", file_name2, GetLastError());
		return false;
	}
	DWORD bytes_read;
	...
	{
		printf("err, ReadFile(%ws) failed, gle = %u", file_name2, GetLastError());
	...
	{
		printf("err, DeleteFile(%ws) failed\n, gle = %u", file_name, GetLastError());
	}
